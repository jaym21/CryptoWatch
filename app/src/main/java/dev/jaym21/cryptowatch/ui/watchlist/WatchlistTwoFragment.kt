...
import dev.jaym21.cryptowatch.databinding.FragmentWatchlistTwoBinding
...
class WatchlistTwoFragment(private val navController: NavController) : Fragment(), ICurrencyRVAdapter {

    private var binding: FragmentWatchlistTwoBinding? = null
    ...
    override fun onCreateView(
        ...
        binding = FragmentWatchlistTwoBinding.inflate(inflater, container, false)
        return binding?.root
    }
    ...
    private fun setUpSwipeToDeleteItem() {
        ...
        itemTouchHelper.attachToRecyclerView(binding?.rvWatchlistTwo)
    }
    ...
    private fun getWatchlistObjectFromCurrencyResponse(currencyResponse: CurrencyResponse): Watchlist? {
        ...
        watchlistViewModel.allCurrenciesInWatchlist.observe(viewLifecycleOwner, Observer {  allWatchlists ->
            allWatchlists.forEach {
                if (it.symbol == currencyResponse.symbol && it.watchlist == "Watchlist 2") {
                    watchlist = it
                    return@Observer
                }
            }
