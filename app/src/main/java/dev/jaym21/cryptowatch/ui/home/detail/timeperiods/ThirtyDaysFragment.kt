...
import dev.jaym21.cryptowatch.databinding.FragmentThirtyDaysBinding
...
class ThirtyDaysFragment(val currencyId: String, val isChangePositive: Boolean) : Fragment() {

    private var binding: FragmentThirtyDaysBinding? = null
    private lateinit var viewModel: CurrencyDetailsViewModel
    private lateinit var customMarkerView: CustomMarkerView
    private var entriesThirtyDays = arrayListOf<Entry>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getCurrencyDailyHistory(currencyId,"30")
        ...
        viewModel.currencyDailyHistory.observe(viewLifecycleOwner, Observer { response ->
            when(response) {
                is ApiResponse.Success -> {
                    ...
                    entriesThirtyDays.clear()
                    response.data?.data?.forEach{
                        entriesThirtyDays.add(Entry(it.time!!.toFloat(), it.close!!.toFloat()))
                    }
                    val dataSet = LineDataSet(entriesThirtyDays, "line chart")
                    updateChart(dataSet)
                }
