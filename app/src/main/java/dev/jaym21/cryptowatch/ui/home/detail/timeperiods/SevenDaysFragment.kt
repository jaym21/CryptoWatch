...
class SevenDaysFragment(val currencyId: String, val isChangePositive: Boolean) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getCurrencyDailyHistory(currencyId,"7")
        ...
        viewModel.currencyDailyHistory.observe(viewLifecycleOwner, Observer { response ->
            when(response) {
                is ApiResponse.Success -> {
                    ...
                    entriesSevenDays.clear()
                    response.data?.data?.forEach{
                        entriesSevenDays.add(Entry(it.time!!.toFloat(), it.close!!.toFloat()))
                    }
                    val dataSet = LineDataSet(entriesSevenDays, "line chart")
                    updateChart(dataSet)
                }
