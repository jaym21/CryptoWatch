...
class OneYearFragment(val currencyId: String, val isChangePositive: Boolean) : Fragment() {
    ...
    private var entriesOneYear = arrayListOf<Entry>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getCurrencyDailyHistory(currencyId,"365")
        ...
        viewModel.currencyDailyHistory.observe(viewLifecycleOwner, Observer { response ->
            when(response) {
                is ApiResponse.Success -> {
                    ...
                    entriesOneYear.clear()
                    response.data?.data?.forEach{
                        entriesOneYear.add(Entry(it.time!!.toFloat(), it.close!!.toFloat()))
                    }
                    val dataSet = LineDataSet(entriesOneYear, "line chart")
                    updateChart(dataSet)
                }
